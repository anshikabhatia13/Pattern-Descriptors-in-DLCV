...
def extract_ltp_features(image, radius=1, n_points=8, threshold=5):
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    height, width = gray.shape
    ltp = np.zeros((height, width), dtype=np.int32)
    ...
    (hist, _) = np.histogram(ltp.ravel(), bins=np.arange(0, 2 * n_points + 3), range=(0, 2 * n_points + 2))
        ...
        if os.path.isdir(class_path):
            for img_name in os.listdir(class_path):
                ...
                if image is not None:
                    feature = extract_ltp_features(image)
...
def retrieve_similar_images(query_image_path, feature_csv, num_results=5):
    ...
    query_feature = extract_ltp_features(query_image).reshape(1, -1)
