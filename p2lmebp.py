...
def extract_lmebp_features(image, radius=1, n_points=8, bit_planes=4):
        ...
        if os.path.isdir(class_path):
            for img_name in os.listdir(class_path):
                ...
                if image is not None:
                    feature = extract_lmebp_features(image)
...
def retrieve_similar_images(query_image_path, feature_csv, num_results=5):
    ...
    query_feature = extract_lmebp_features(query_image).reshape(1, -1)
...
if __name__ == "__main__":
    ...
    feature_csv = "proposed_localmean_features.csv"
